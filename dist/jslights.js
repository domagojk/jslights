...
var jsLights = (function (_ModuleEventEmitter) {
  ...
  _createClass(jsLights, [{
    ...
    key: '_assign',
    value: function _assign(path, assign) {
      assign.jsLights = {
        path: path
      };
      ...
      try {
        for (var _iterator = components[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (!pointer[component]) {
            if (i == components.length) pointer[component] = assign;else pointer[component] = {};
          }
    ...
    key: '_onPassedDependencies',
    value: function _onPassedDependencies(params, cb) {
      var _this2 = this;
      ...
      var checkDeps = function checkDeps(events, initial) {
        // if class is already instantiated
        if (_this2.triggered.indexOf(path) != -1) {
          return;
        }

        var deps = [];
        events.forEach(function (dependency) {
          if (initial && dependency.indexOf('->') != -1) {
            var eventsArr = dependency.split('->');
            var baseClass = eventsArr[0];
            var onBaseClass = eventsArr[1];

            _this2.onPathInstantiated(baseClass, function () {
              var baseClassObj = _this2._getPropertyByPath(baseClass);
              if (!baseClassObj) {
                ...
                if (baseClassObj.jsLights && baseClassObj.jsLights.triggered && baseClassObj.jsLights.triggered[onBaseClass]) {
                  _this2.registerEvent(dependency);
                } else {
                  baseClassObj.on(onBaseClass, function () {
                    _this2.registerEvent(dependency);
                  });
                }
              }
          ...
          if (_this2.triggered.indexOf(dependency) == -1) {
